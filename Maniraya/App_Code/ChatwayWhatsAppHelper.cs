...
public static class ChatwayWhatsAppHelper
{
    ...
    public static bool TrySendInvoiceAfterApprove(string userId, string orderId, InvoiceMessageType messageType, out string statusMessage)
    {
        ...
        // file_url=.../SavingProductInvoice.aspx?orderId=...  (no accessKey) and file_name=Invoice
    ...
    public static bool TrySendFile(string number, string message, string fileUrl, string fileName, out string statusMessage)
    {
        ...
        try
        {
            ...
            if (!ok && !isStaticPdf && IsGenericChatwayError(responseText))
            {
                ...
                ok = ExecuteSendFileRequest(username, token, apiBase, numberWithPlus, message, simpleUrl, "Invoice", timeoutMs, out responseText);
            }
        ...
        out string responseText)
    {
        ...
        query["file_name"] = string.IsNullOrWhiteSpace(fileName) ? "Invoice" : fileName.Trim();
