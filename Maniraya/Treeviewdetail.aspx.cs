...
public partial class Treeviewdetail : System.Web.UI.Page
{
    ...
    public static List<User> GetChildren(string parentId)
    {
        ...
        {
            string query = "SELECT userid, username, parentuserid AS parentid, CASE WHEN ISNULL(status,0)=1 THEN 'Active' ELSE 'Inactive' END AS status, CASE WHEN ISNULL(status,0)=1 THEN 'user/img/green-boy.png' ELSE 'user/img/red-boy.png' END AS photoimage, (SELECT ISNULL(SUM(planamount),0) FROM UserTopupTb WHERE Userid=u.userid) AS topupamount FROM UserDetail u WHERE u.sponserid=@parentid;";
